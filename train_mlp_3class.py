...
def train_mlp(X, y, groups):
    ...
    np.save(rf"{BASE}\models\mlp_oof_ytrue.npy", np.array(y_true_all))
    np.save(rf"{BASE}\models\mlp_oof_yproba.npy", np.array(y_proba_all))
    ...
    pickle.dump({'model_state': final.state_dict(),
                 ...
                 open(rf"{BASE}\models\mlp_model.pkl", 'wb'))
    return metrics
